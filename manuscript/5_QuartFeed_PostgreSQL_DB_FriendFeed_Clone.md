...
![A plain def wrapper returns a coroutine Quart never awaits, so the view never runs; an async def wrapper is awaited properly, so the view runs.](images/5.5-scene2-img2.png)
